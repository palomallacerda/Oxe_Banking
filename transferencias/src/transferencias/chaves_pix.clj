...
(defn gerar-chave-pix []
    ...
    (defn criar_chave_cnpj []
        (let [chave_cnpj (read-line)] ;criando chave com o numero de cnpj
            (def cnpjs_cadastrados
                {:cnpj_criado chave_cnpj} ;inserindo cpf criado no mapa de cnjps cadastrados
            )))
    ...
    (println "digite 2 para gerar chave com numero de CNPJ")
    (println "digite 3 para gerar chave com endereco de e-mail")
    (println "digite 4 para gerar chave com numero de celular")
    (println "digite 5 para gerar chave com um numero aleatorio")
    (let [chave (read-line)] ;chave
        ...
        (if (== 1 numero) ;condicionais para saber o tipo de chave que foi escolhida
            ...
            (if (== 2 numero)
                ;(println "Opcao De Chave Escolhida: CNPJ")
                (let [novo_cnpj (criar_chave_cnpj)] novo_cnpj)
